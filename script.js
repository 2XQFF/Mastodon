...
class Game {
    ...
    canPlaceTile(tile, targetQ, targetR) {
        ...
        if (!this.impossible_declared && this.placedTilesThisTurn > 0) { //
            ...
            if (proposedCoords.length === 2) {
                ...
                // 3개째 타일: 이전(2개) 타일 중 하나에 인접해야 함
                const [p1, p2] = currentTurnTilesCoords; // 이미 놓인 두 타일
                const p3 = { q: targetQ, r: targetR }; // 새로 놓을 세 번째 타일

                let p3IsAdjacentToP1 = false;
                for(let i=0; i<6; ++i) {
                    const {q:nq, r:nr} = getNeighborHex(p1.q, p1.r, i);
                    if(nq === p3.q && nr === p3.r) {
                        p3IsAdjacentToP1 = true;
                        break;
                    }
                }
                let p3IsAdjacentToP2 = false;
                for(let i=0; i<6; ++i) {
                    const {q:nq, r:nr} = getNeighborHex(p2.q, p2.r, i);
                    if(nq === p3.q && nr === p3.r) {
                        p3IsAdjacentToP2 = true;
                        break;
                    }
                }

                if (!(p3IsAdjacentToP1 || p3IsAdjacentToP2)) {
                    this.showMessage("3개째 타일은 이번 턴에 놓은 이전 타일 중 하나에 인접해야 합니다.");
                    return false;
                }
                // 실제 Railroad Ink의 "3개 타일 일렬 규칙"은 더 복잡합니다.
                // 여기서는 간단히 인접성만 확인했습니다.
            }
        }
    ...
    // 승리 조건 확인 (완전한 회로 감지)
    checkWinCondition() {
        // 보드에 놓인 모든 타일의 모든 가능한 시작 포트에서 탐색 시작
        // 시작역 타일 (0,0)과 (1,0)도 포함하여 탐색합니다.
        for (const tileKey of this.board.keys()) {
            const [q, r] = tileKey.split(',').map(Number);
            const tile = this.board.get(tileKey);

            // 각 타일의 현재 회전에 따른 유효한 연결 포트 쌍을 순회합니다.
            // Tile.getCurrentLocalConnections()는 이미 타일의 회전이 반영된 상태의 '전역 포트 인덱스'를 반환하므로,
            // findCircuit에 전달할 때 이 인덱스를 바로 사용합니다.
            for (const [portA_global, portB_global] of tile.getCurrentLocalConnections()) {
                // portA_global에서 시작하는 경우
                if (this.findCircuit(q, r, portA_global)) {
                    console.log(`승리 조건 달성! 회로가 (${q}, ${r})의 ${portA_global} 포트에서 발견되었습니다.`);
                    return true; 
                }
                // portB_global에서 시작하는 경우 (양방향 연결이므로)
                if (this.findCircuit(q, r, portB_global)) {
                    console.log(`승리 조건 달성! 회로가 (${q}, ${r})의 ${portB_global} 포트에서 발견되었습니다.`);
                    return true; 
                }
            }
        }
